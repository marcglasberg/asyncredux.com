...
* **`VmFactory`**: Creates the view-model from the store state, by separating and transforming the state.
* **`StoreConnector`**: Connects the store to the dumb widget, by using the factory to create
  the view-model, and then passing the view-model to the dumb widget.
