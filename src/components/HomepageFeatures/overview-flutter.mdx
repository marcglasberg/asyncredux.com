...
By default, runs never overlap: each tick waits for the previous run to finish,
so the interval is measured from the end of each run.
Override `pollWaitsForRun` to `false` if you want ticks at a fixed rate instead.

&nbsp;

## Sequential

Mixin `Sequential` makes actions run one at a time, in the exact order they were dispatched.
All actions using the mixin share a FIFO queue, and each one waits for the previous to finish.
Override `sequentialKeyParams` if you want independent queues.
...
class SaveItem extends AppAction with Sequential {
  ...
  // Each different `itemId` has its own queue.
  Object? sequentialKeyParams() => item.id;
...
You can add a `stateObserver` to collect app metrics,
an `actionObserver` to print information to the console during development,
and a `globalErrorObserver` to catch and log all errors.

```dart
var store = Store<String>(
  stateObserver: [MyStateObserver()],
  actionObservers: [MyActionObserver()],
  globalErrorObserver: (_) => MyGlobalErrorObserver(),
```

&nbsp;

For example, the following `globalErrorObserver` handles `PlatformException` errors thrown
